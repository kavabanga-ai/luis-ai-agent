...
from retrieval_graph.utils import create_service_with_proxy
...
def make_text_encoder(model: str) -> Embeddings:
    ...
    match provider:
        case "openai":
            ...
            return create_service_with_proxy(OpenAIEmbeddings, model)
        case "cohere":
            from langchain_cohere import CohereEmbeddings

            return create_service_with_proxy(CohereEmbeddings, model)
...
def make_elastic_retriever(
    ...
    # Safely access search_kwargs with a default empty dictionary if not present
    search_kwargs = getattr(configuration, 'search_kwargs', {})

    # Validate the presence of user_id in configuration and its validity
    user_id = getattr(configuration, 'user_id', None)
    if not user_id:
        raise ValueError("Please provide a valid user_id in the configuration.")

    # Initialize or retrieve the 'filter' key in search_kwargs as a list to store filter conditions
    search_filter = search_kwargs.get('filter', [])
    if not isinstance(search_filter, list):
        search_filter = []

    # Add a new filter condition to the list of search filters
    search_filter.append({"term": {"metadata.user_id": user_id}})

    # Ensure the updated list of filters is assigned back to search_kwargs
    search_kwargs['filter'] = search_filter

    # Assume vstore is already defined and properly configured elsewhere in your code
...
def make_pinecone_retriever(
    ...
    # Validate the user_id is provided in configuration and is valid
    user_id = getattr(configuration, 'user_id', None)
    if not user_id:
        raise ValueError("Please provide a valid user_id in the configuration.")

    # Safely retrieve or initialize the 'filter' key in search_kwargs
    search_filter = search_kwargs.get('filter', {})
    search_filter.update({'user_id': user_id})

    # Ensure the 'filter' is updated back into search_kwargs
    search_kwargs['filter'] = search_filter

    # Initialize the PineconeVectorStore with the environment's index name and embedding model
...
def make_mongodb_retriever(
    ...
    # Safely access search_kwargs and ensure they exist using getattr with a default empty dictionary
    search_kwargs = getattr(configuration, 'search_kwargs', {})
    ...
    # Safely handle the 'pre_filter' key in search_kwargs
    pre_filter = search_kwargs.get('pre_filter', {})
    pre_filter['user_id'] = {'$eq': user_id}

    # Update the 'pre_filter' back into search_kwargs if it was newly created or modified
...
def make_milvus_retriever(
    ...
    vstore = Milvus(
        ...
        auto_id=True,
        collection_name="langchain_collection"  # Specify your collection name
    )
