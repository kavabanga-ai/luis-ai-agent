...
def load_chat_model(fully_specified_name: str, response_model_kwargs: Optional[dict] = None) -> BaseChatModel:
    ...
    if provider == "perplexity":
        ...
        return ChatPerplexity(model=model, temperature=0)

    # Prepare common parameters for initializing the model
    base_url = os.getenv("VSEGPT_LLM_API_BASE_URL")
    ...
    if provider.lower() == "ollama":
        ...
        auth_token_ollama = os.getenv("OLLAMA_BASE_URL_TOKEN")
        if auth_token_ollama:
            common_params["client_kwargs"] = {"headers": {"Authorization": f"Bearer {auth_token_ollama}"}}
